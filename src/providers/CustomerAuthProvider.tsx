...
export function CustomerAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const clearAuthCache = useCallback(() => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('zolvo_customer_user');
      localStorage.removeItem('zolvo_customer_profile');
      document.cookie = 'zolvo_customer_uid=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
      document.cookie = 'zolvo_customer_role=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
    }
  }, []);
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    try {
      ...
      if (profileData) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('zolvo_customer_profile', JSON.stringify(profileData));
          document.cookie = `zolvo_customer_role=${profileData.role}; path=/; max-age=2592000;`;
        }
      }
      return profileData;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    async function setupAuth() {
      try {
        if (typeof window !== 'undefined') {
          const cachedUserJson = localStorage.getItem('zolvo_customer_user');
          const cachedProfileJson = localStorage.getItem('zolvo_customer_profile');
      ...
      const { data: { subscription: sub } } = supabase.auth.onAuthStateChange(async (event: AuthChangeEvent, session: Session | null) => {
        if (!active) return;
        logger.info(`[CustomerAuthProvider] Supabase Auth Event: ${event}`);
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('zolvo_customer_user', JSON.stringify({ id: supabaseUser.id, email: supabaseUser.email }));
          document.cookie = `zolvo_customer_uid=${supabaseUser.id}; path=/; max-age=2592000;`;
        }
