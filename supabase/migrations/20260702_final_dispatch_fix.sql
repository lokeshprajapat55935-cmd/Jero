...
-- 
-- Root Cause:
--   The notify_nearby_workers() function references w.city_id 
--   which does not exist in the public.workers table.
--
-- Fix:
--   Use public.worker_locations.city_id as the source for 
--   city filtering.
-- ============================================================
...
BEGIN
  -- 1. Get dispatch response window from platform configuration
  ...
  -- 2. Identify the active dispatch request for this booking
  SELECT id 
  INTO v_dispatch_id 
  ...
  -- 3. Find eligible workers based on category, status, city, and proximity
  FOR v_worker IN
    SELECT w.id
    FROM public.workers w
    -- CRITICAL FIX: Join with worker_locations to get city_id as workers table lacks it
    ...
    WHERE w.status = 'approved'
      ...
      -- Use worker_locations.city_id for city filtering
      AND wl.city_id = p_city_id
      -- Ensure worker hasn't already been notified for this specific dispatch request
      AND (
      ...
      -- Optional distance-based filtering if coordinates are provided
      AND (
        (
    ...
    ORDER BY
      -- Sort by distance (if coordinates available), then by rating
      CASE 
  ...
  LOOP
    ...
    -- 5. Send notification (triggers real-time update to the worker)
    ...
    -- 6. Update dispatch request with the current targeted worker
    IF v_dispatch_id IS NOT NULL THEN
  ...
  -- 7. Update the booking's cumulative notified worker count
